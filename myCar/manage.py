...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    #获取GPS误差信息
    #from donkeycar.parts.get import get
    #V.add(get(),outputs=['error/longitude','error/latitude'])
    ...
    if cfg.USE_ULTRASONIC:
        ...
        pi = pigpio.pi()
        from donkeycar.parts.sonicrangesensor import Sensor   #传感器类
    ...
    pilot = PID_Pilot(pid, cfg.PID_THROTTLE, cfg.USE_CONSTANT_THROTTLE, min_throttle=cfg.PID_THROTTLE, use_ultrasonic=cfg.USE_ULTRASONIC, min_distance=cfg.MIN_DISTANCE, min_distance_start=cfg.MIN_DISTANCE_START)
    V.add(pilot, inputs=['cte/error', 'throttles', 'cte/closest_pt','range/cms'], outputs=['pilot/steering', 'pilot/throttle'], run_condition="run_pilot")
...
def add_gps(V, cfg):
    if cfg.HAVE_GPS:
        ...
        from donkeycar.parts.gps import GpsNmeaPositions, GpsLatestPosition, GpsPlayer, RTK
        ...
        #RTK修复误差
        V.add(RTK(),inputs=['gps/longitude', 'gps/latitude','error/longitude','error/latitude'],outputs=['gps/longitude', 'gps/latitude','gps/utm/longitude', 'gps/utm/latitude'])
