...
class CTE(object):
    ...
    def nearest_waypoints(self, path, x, y, look_ahead=1, look_behind=1, from_pt=0, num_pts=None):
        ...
        a = (i + len(path) - look_behind) % len(path) if i > 1 else 0 

        # get the end of the segment
        #b = (i + look_ahead) % len(path)
        b = (i + look_ahead) % len(path) if i < len(path) - 3 else len(path) - 1
...
class PID_Pilot(object):
    ...
    def __init__(
            ...
            use_ultrasonic: bool = False,
            min_distance : float = 0,
            min_distance_start : float = 0) -> None:
        self.pid = pid
        ...
        self.min_distance_start = min_distance_start
        self.obstacle = False

    def run(self, cte: float, throttles: list, closest_pt_idx: int, distance: float) -> tuple:
        ...
        if distance is not None:
            if self.obstacle is False and distance <= self.min_distance or self.obstacle is True and distance <= self.min_distance_start:
                throttle = 0
                self.obstacle = True
            else:
                self.obstacle = False
