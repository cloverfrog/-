...
class RTK:
    def __init__(self):
        self.fixed_lat = 0
        self.fixed_lon = 0
    def run(self,lon,lat,err_lon,err_lat):
        if lon is not None and lat is not None:
            self.fixed_lon = lon #- err_lon
            self.fixed_lat = lat #- err_lat
            #print("修正数据",self.fixed_lon,self.fixed_lat,lon,lat)
        utm_position = utm.from_latlon(self.fixed_lat, self.fixed_lon)
        return self.fixed_lon,self.fixed_lat,float(utm_position[0]), float(utm_position[1])
